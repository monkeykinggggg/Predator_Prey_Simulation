...
class Sound(mesa.Agent):
    def __init__(self, model: mesa.Model, r: int, direction: Direction, edge: bool = False, force: float = None):
        ...
        self.force = force if force is not None else 10 / r ** 2
        self.r = r
        self.edge = edge    # when it is initialed on the edge of the sound wave, it should propagate in two parpendicular directions, only for new sound agents (one square)
        self.direction = direction

    @staticmethod
    def create_sound(model: mesa.Model, pos: Tuple[int, int], radius: int, direction: Direction, edge: bool = True, force: float = None)-> None:
    ...
    def update_value(self) -> None:
        """Updates current radius and force of the sound."""
        self.r += 1
        self.force = self.force / self.r ** 2
