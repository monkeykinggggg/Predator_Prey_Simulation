...
class Pheromone(mesa.Agent):
    ...
    def update_value(self) -> None: 
        self.value = (1 - self.evaporation_rate) * self.value + self.diffusion_rate * (self.avg_value - self.value)
    ...
    def step(self) -> None:
        ...
        for pos in positions:
            if self.model.grid.is_cell_empty(pos):
                Pheromone.create(self.model,pos, self.value)    # pusty sasiad rozprzestrzenia feromon
            else:
                neighbor = self.model.grid[pos]
                if type(neighbor) is Pheromone:
                    neighbor.value = self.value # albo aktalizuje wartosc do naszej
